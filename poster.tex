...
% change alert text color
\definecolor{plotlyblue}{HTML}{1f77b4}
\definecolor{cugold}{HTML}{CFB87C}
\definecolor{mybrickred}{HTML}{d62728}
\setbeamercolor{alerted text}{fg=mybrickred}
%% Hyperparameters
\newcommand\myshade{85}
\colorlet{mylinkcolor}{violet}
\colorlet{mycitecolor}{plotlyblue}
...
\newcommand{\phatCMAP}{\hat{{\color{mybrickred}\mathbf{p}}}_{\mathrm{C-MAP}}}
%% On the continuum
...
\begin{tikzpicture}[remember picture, overlay]
    % (0,0) is the center of the page by default with 'current page.center'
    \node at (current page.north) [shift={(8cm, -4cm)}] {
        \includegraphics[width=3cm]{figures/qrcode.pdf}
    };
\end{tikzpicture}
\begin{tikzpicture}[remember picture, overlay]
    % (0,0) is the center of the page by default with 'current page.center'
    \node at (current page.north) [shift={(2.2cm, -4.75cm)}] {
        \includegraphics[width=.22\textwidth]{figures/logo.pdf} 
    };
\end{tikzpicture}
...
\begin{block}{Approximate Distribution}
    ...
    \end{equation}
    \normalfont
    where an analytic expression for $\S$ follows from the linearization and the noise distribution, see \citep{RummelMessengerBeckerEtAl2025arXiv250208881b}.
...
\begin{block}{Consistency}
    ...
    \begin{equation}\label{eq:consistency}
        \lim_{M\rightarrow \infty} \mathbb{E}[\phatCMAP ]= \p^*.
    \end{equation}\vspace{-3mm}
...
\begin{exampleblock}{Example: Logistic Growth}
    ...
    \begin{equation} \label{eq:logisticGrowth}
            \dot{u} = {\color{ForestGreen} p}_1 u - {\color{ForestGreen} p}_2 u^2
    \end{equation}
