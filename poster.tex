...
\author{N. Rummel\inst{1}, S. Becker\inst{1}, V. Dukic\inst{1}, D. Bortz\inst{1}}
\institute{\inst{1} Department of Applied Mathematics, University of Colorado Boulder
...
\begin{block}{Approximate Distribution}
    On expectation, the \alert{weak residual} should be small 
    \small
...
\begin{block}{Maximum a Posteriori Estimator}
    Often a \alert{prior} for the parameters $\p \sim \Pi$ is known or one can utilize an \emph{objective prior} \citep{JordanMcLoughlin,Robert2007,ChoiRamamoorthi2008InstituteofMathematicalStatisticsCollections}. 
...
\begin{exampleblock}{Example: Logistic Growth}
    ...
    % \end{center}
    \normalfont
    one can now estimate for $\p$ with \cref{eq:opt} and \cref{eq:corrector}. 
    \begin{figure}[H]
        \centering
    ...
    \begin{table}
    \begin{tabular}{llc}
        \textbf{Objective} && \textbf{Relative Error}\\
        \midrule
